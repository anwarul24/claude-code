...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $update = $conn->prepare("UPDATE students SET name=?, roll=?, email=?, phone=?, department=?, session=? WHERE id=?");
        $update->bind_param('ssssssi', $old['name'], $old['roll'], $old['email'], $old['phone'], $old['department'], $old['session'], $id);
        if ($update->execute()) {
    ...
    <form method="POST" action="edit.php?id=<?php echo $id; ?>" novalidate>
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <input type="text" name="name" class="form-control" value="<?php echo htmlspecialchars($old['name']); ?>">
            </div>
            <div class="col-md-6">
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="department" class="form-control" value="<?php echo htmlspecialchars($old['department']); ?>">
            </div>
            <div class="col-md-6">
                <label class="form-label">Session</label>
                <input type="text" name="session" class="form-control" value="<?php echo htmlspecialchars($old['session']); ?>">
            </div>
        </div>

        <div class="mt-4 d-flex gap-2">
            <button type="submit" class="btn btn-brand px-4"><i class="fa-solid fa-check me-1"></i> Update Student</button>
